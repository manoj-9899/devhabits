...
function validateHabitBody(body, requireName = true) {
  ...
  if (requireName && (!name || typeof name !== 'string' || !name.trim())) {
    throw createError(
      400,
      'NAME_REQUIRED',
      'Habit name is required and must be a non-empty string.'
    );
  }
  ...
  if (color && !VALID_COLORS.test(color)) {
    throw createError(
      400,
      'INVALID_COLOR',
      'color must be a valid 6-digit hex string (e.g. #6366f1).'
    );
  }
}
...
router.get(
  ...
  asyncHandler((req, res) => {
    ...
    let habits = category ? HabitModel.getHabitsByCategory(category) : HabitModel.getAllHabits();
